...
describe('Helix Config', () => {
  ...
  it('loads an config with references', async () => {
    const cfg = await new HelixConfig()
      .withConfigPath(path.resolve(SPEC_ROOT, 'refs.yaml'))
      .init();

    const expected = JSON.parse(await fs.readFile(path.resolve(SPEC_ROOT, 'refs.json'), 'utf-8'));
    const actual = cfg.toJSON();
    assert.deepEqual(actual, expected);
  });
