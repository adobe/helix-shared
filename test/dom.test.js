...
const {
  ...
  isNode, assertNode, nodeName, nodeMatches, recursiveDomDump,
...
describe('dom equivalence nodeIsEquivalent(), assertEquivalentNode(), nodeMatches()', () => {
  ...
  it('recursiveDomDump works', async () => {
    ...
    assert.throws(() => recursiveDomDump(prebase.window.document, prechanges.window.document, 0));
  });

  it('recursiveDomDump equality tests', async () => {
    each([docA, docB, docC], (v) => {
      recursiveDomDump(v.window.document, docD.window.document);
    });
  });
});
