...
const { compose } = require('../src/functional');
const {
  typedArrays, isdef, type, typename, isPrimitive, HybridWeakMap,
  TraitNotImplemented, Trait, implements, valueImplements, typeIsImmutable,
...
describe('Trait', () => {
  ...
  it('implements()', () => {
    assert(implements(String, Foo));
    assert(implements(Object, Foo));
    assert(implements(Bar, Foo));
    assert(implements(Baz, Foo));
    assert(!implements(Number, Foo));
  });
