...
- **helix-shared-queue-sqs** — Default AWS SQS `QueueBackend` for `helix-shared-queue`; `QueueServiceSqs` is the pre-wired `QueueService` subclass. A behavior-preserving port of `BatchedQueueClient` (`@adobe/helix-admin-support`): batches `send`/`delete` to SQS's own limits, long-polls `receive`, and spills oversized messages to an injected `@adobe/helix-shared-storage` `Bucket` instead of hardcoding S3.
