...
- **helix-shared-queue-servicebus** — Azure Service Bus `QueueBackend` for `helix-shared-queue`; `QueueServiceServiceBus` is the pre-wired `QueueService` subclass. No native partial-batch-failure reporting or bulk-ack (unlike SQS), so `sendBatch`/`deleteBatch` differ structurally from the SQS backend; spills oversized messages to an injected `@adobe/helix-shared-storage` `Bucket`, transparently dereferenced on receive, same pattern as `helix-shared-queue-sqs`.
