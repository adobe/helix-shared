 ...
 *  resolve the spill bucket for every queue this factory hands out; see {@link SqsBackend}.
 *  Can be overridden per queue via `{storage}` passed to `QueueService#queue()`.
 * @property {string} [bucketName] default spill bucket name; see {@link SqsBackend}. Can be
 *  overridden per queue via `{bucketName}` passed to `QueueService#queue()`.
