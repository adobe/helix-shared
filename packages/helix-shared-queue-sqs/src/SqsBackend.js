 ...
 *  to resolve the spill bucket (by `bucketName`) for a single message too large to fit in
 *  any batch on its own. When omitted, attempting to send such a message throws.
 ...
 * contain `owner`/`repo` (or an explicit `key`) fields, and requires the configured `storage`
 ...
 * directly, bypassing this package's storage abstraction entirely.
 *
...
export class SqsBackend extends AbstractQueueBackend {
   ...
   */
  constructor({
    client, queueName, log = console, storage, bucketName,
   ...
   * bucket (resolved via `storage.bucket(bucketName)`), replacing its body with a pointer
   * matching `BatchedQueueClient.serialize()`'s exact wire format.
   *
  ...
  async #spill(entry) {
    ...
    await bucket.put(swapKey, entry.MessageBody, 'application/json', {}, false);
    const swapS3Url = `s3://${this.#bucketName}/${swapKey}`;
   ...
   * (still a pointer) — see `QueueServiceSqs#isSwapped`/`#deserialize`.
   *
   ...
   * {@link SqsBackend#deleteBatch}'s post-ack cleanup to use later. Consumers checking
   * spillover on the returned message use `QueueServiceSqs#isSwapped`/`#deserialize` instead,
   * which resolve directly from `message.body` rather than this internal stash.
   *
   ...
   * `QueueServiceSqs#deserialize` was ever called for it.
   *
   ...
   * `QueueServiceSqs#deserialize` happens to have been called) so a message that's never
