 ...
 * `{owner, repo, key, swapS3Url}` shape) and, if so, extracts its storage bucket/key. Exported
 * so {@link SqsBackend} can reuse this same detection logic for its own internal receive-time
 * bookkeeping without needing to also import the (fetching) {@link dereferenceMessageBody}.
 *
 * @param {Object} parsed the parsed message body
 * @returns {{swapBucket: string, swapKey: string}|undefined} the pointer's bucket/key
 *  (parsed out of its `swapS3Url`), if `parsed` is a swap pointer
 */
export function extractSwapKey(parsed) {
  ...
  return { swapBucket: hostname, swapKey: pathname.substring(1) };
}

/**
 * Cheap (no I/O) check for whether `body` is a swap pointer produced by
 * {@link SqsBackend#sendBatch} rather than the real message content.
 *
 ...
 *
 * Trusts the pointer's own `swapS3Url` bucket to resolve the bucket via `storage.bucket(...)` —
