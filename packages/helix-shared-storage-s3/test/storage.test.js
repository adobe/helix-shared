...
function buildTestStorage(opts = {}) {
  ...
  const backendFactory = (bucketId, { disableR2 } = {}) => {
    const s3Backend = new S3Backend({
      client: s3Client, name: 'S3', bucketName: bucketId, log: console,
    });
    if (disableR2) {
      return s3Backend;
    }
    const r2Backend = new S3Backend({
      client: r2Client, name: 'R2', bucketName: bucketId, log: console,
    });
...
describe('Storage test', () => {
  ...
  it('can get an object with a different bucket name', async () => {
    ...
    storage = buildTestStorage({ bucketNames: JSON.stringify(map) });
  ...
  it('can get the s3 client', () => {
    assert.ok(storage.codeBus().client);
  });
  ...
  it('remove can report a non XML error from R2', async () => {
    // Each backend now normalizes its own delete errors internally (S3Backend#remove) before
    // MirroringBackend tags the failing leg by name, so the tag is applied after normalization
    ...
    nock('https://helix-code-bus.s3.fake.amazonaws.com')
      ...
      .reply(function f(uri) {
        putsHeaders.s3 = {
          'content-type': this.req.headers['content-type'],
          'content-encoding': this.req.headers['content-encoding'],
          'cache-control': this.req.headers['cache-control'],
          'content-disposition': this.req.headers['content-disposition'],
          'x-amz-metadata-directive': this.req.headers['x-amz-metadata-directive'],
          'x-amz-meta-x-dont-overwrite': this.req.headers['x-amz-meta-x-dont-overwrite'],
  ...
  it('can copy object can fail if not found (non deep)', async () => {
    // Note: the `<Key>` must be nested inside `<Error>` (as in AWS's real error schema) rather
    // than a second, sibling root element — a document with two root elements is invalid XML
    // and is rejected by the SDK's XML parser as a deserialization error before it ever gets a
    // chance to read `Code`, which would mask the `NoSuchKey` normalization path entirely.
    const body = '<?xml version="1.0" encoding="UTF-8"?><Error><Code>NoSuchKey</Code><Key>/owner/repo/ref/foo.md</Key></Error>';
    nock('https://helix-code-bus.s3.fake.amazonaws.com')
      .put('/owner/repo/ref/foo/bar.md?x-id=CopyObject')
      .reply(200, body);
    nock(`https://helix-code-bus.${CLOUDFLARE_ACCOUNT_ID}.r2.cloudflarestorage.com`)
